...
class Cadillac : public Automatic {
	int year;
	int km;
public:
	Cadillac(string vendor, string model, string engineNo, string company, double volume, int year, int km) : Automatic(vendor, model, engineNo, company, volume) {
		SetYear(year);
		SetKm(km);
	}
...
void main() {
	...
	Car* car2 = new Mercedes("Mercedes", "GLC200", "X253", "Mercedes", 5.6, 2019, 100000);
	Car* car3 = new Cadillac("Cadillac", "Escalade", "L86", "Cadillac", 6.2, 2022, 0);
